...
- 主流程模型通过 `LLMProvider` 接入；`GatewayLLMProvider` 使用 Anthropic Messages Tool Use 返回结构化结果，协议和供应商信息不进入领域层。
...
- `pfinder-ai investigate` 默认使用合成数据源且禁用真实模型；本地显式配置后，仅 ClueExtractor 调用模型网关。
...
- GatewayLLMProvider 已实现强制 Tool Use、Pydantic 校验、一次格式修复、有限重试、错误脱敏和 UsageMonitor 接入；合成 Tool Use Smoke Test 已通过。
...
1. 整理公司 Metadata、Pfinder 和日志 API 能力与字段。
2. 实现对应真实 Adapters 和 Contract Tests，并继续验证模型网关的企业级服务边界。
